...
class Object_Sync_Sf_Salesforce {
	...
	protected function api_http_request( $path, $params, $method, $options = array(), $type = 'rest' ) {
		...
		if ( $this->debug === true ) {
			...
			if ( function_exists( 'str_ends_with' ) ) {
				if ( str_ends_with( $query_key, $query_end ) === true ) {
					$is_soql_query = true;
				}
			} else {
			...
			$title = sprintf(
				...
				( $is_soql_query === false ) ? esc_html__( 'There is not an SOQL query included in this request.', 'object-sync-for-salesforce' ) : esc_html__( 'There is an SOQL query included in this request.', 'object-sync-for-salesforce' )
			);
			$body = sprintf(
	...
	public function run_analytics_report( $id, $async = true, $clear_cache = false, $params = array(), $method = 'GET', $report_cache_expiration = '', $cache_instance = true, $instance_cache_expiration = '' ) {
		...
		if ( $async === false || $instance_id === false ) {
